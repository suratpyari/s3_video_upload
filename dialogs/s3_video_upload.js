    ...
    success: function(){
      ...
      xhr1 =  $.ajax({
          ...
          success: function(response){
            ...
            i.setAttribute("data-video-id",response.id)
            i.setAttribute("data-video-url",response.video_url)
            i.src = file_url
            // show delete button
  ...
  $.ajax({
    ...
    success: function(response){
      ...
      $.map(response, function(response){
        ...
        im.setAttribute("data_video_url",response.video_url)
        ...
        $("#"+response.id).on('click', function(){
          var i = get('my_image')
          i.style.display = "block";
          i.setAttribute("data-video-id",this.id)
          i.src = this.src
          i.setAttribute("data-video-url",this.getAttribute('data_video_url'))
